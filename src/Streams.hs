    ...
    action consume produce = do
        ...
        let act1 = streamAction a
            ...
            consume2 = \checkpoint -> do
                mVal <- liftIO $ atomically $ takeTMVar chan
                case mVal of
            ...
            withAsync (runExceptT (act1 consume produce1)) $ \left -> do
              withAsync (runExceptT (act2 consume2 produce)) $ \right -> do
                ...
                result <- case rv of
                    ...
                    -- on it's own.
                    Left result -> do
                        atomically $ putTMVar chan Nothing
                        _ <- wait right
                        return result
...
runStream s = runExceptT action >>= either (\_ -> fail "streaming error") return
  where
